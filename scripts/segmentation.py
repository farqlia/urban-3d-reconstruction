...
def run_chunked(segmentor_ckpt_path, input_chunks_dir, output_ply_path):
    model = PointNetSegmentor.load_from_checkpoint(segmentor_ckpt_path, strict=False)
    dataset = ChunkedPointCloudDataset(input_chunks_dir, SAMPLE_SIZE)
    data_loader = DataLoader(dataset)
    trainer = pl.Trainer()
...
def merge_chunked_with_labels_in_ply(preds, input_chunks_dir, output_ply_path):
    ...
    for idx in range(len(os.listdir(input_chunks_dir))):
        ...
        chunk_cloud = PyntCloud.from_file(f"{input_chunks_dir}/chunk_{idx}.csv").points[['x', 'y', 'z']] # chunk[0][0].cpu().numpy().transpose()
    ...
    new_cloud = pd.DataFrame(np.concatenate([merged_cloud, predictions[..., np.newaxis]], axis=1), columns=['x', 'y', 'z', 'class_label'])
